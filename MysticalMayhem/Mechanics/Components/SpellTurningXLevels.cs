...
namespace MysticalMayhem.Mechanics.Components
{
    ...
    public class SpellTurningXLevels : UnitFactComponentDelegate, ITargetRulebookHandler<RuleCastSpell>, IRulebookHandler<RuleCastSpell>, ISubscriber, ITargetRulebookSubscriber
    {
        ...
        public void OnEventDidTrigger(RuleCastSpell evt)
        {
            ...
            if (!evt.Success || result == null || evt.SpellTarget.Unit == evt.Initiator) return;             if (_totalLevels <= 0 || evt.Spell.Range == Kingmaker.UnitLogic.Abilities.Blueprints.AbilityRange.Touch) return;             evt.SetSuccess(false);
            evt.CancelAbilityExecution();
            if (evt.Spell.SpellLevel <= _totalLevels)
            {
                var context = result.Context.CloneFor(Owner, evt.Initiator);
